...
const InputBase:ForwardRefRenderFunction<HTMLInputElement, InputProps> = ({ password = false, error = null, label, className, ...rest }, ref) => {  
                ...
                password?
                    <div className="relative w-full">
                        ...
                        <div className="absolute inset-y-0 right-0 pr-3 flex items-center text-sm leading-5 cursor-pointer">
                            <svg className={`h-6 dark:text-gray-400 text-gray-700 fill=none ${showPassword&&'invisible hidden'} ${!showPassword&&'block'}`} fill="none" xmlns="http://www.w3.org/2000/svg" onClick={()=>setShowPassword(!showPassword)}
                            viewBox="0 0 576 512">
                                <path fill="currentColor"
                                    d="M572.52 241.4C518.29 135.59 410.93 64 288 64S57.68 135.64 3.48 241.41a32.35 32.35 0 0 0 0 29.19C57.71 376.41 165.07 448 288 448s230.32-71.64 284.52-177.41a32.35 32.35 0 0 0 0-29.19zM288 400a144 144 0 1 1 144-144 143.93 143.93 0 0 1-144 144zm0-240a95.31 95.31 0 0 0-25.31 3.79 47.85 47.85 0 0 1-66.9 66.9A95.78 95.78 0 1 0 288 160z">
                                </path>
                            </svg>
                            <svg className={`h-4 md:h-6 dark:text-gray-400 text-gray-700 ${!showPassword&&'invisible hidden'} ${showPassword&&'block'}`} fill="none" xmlns="http://www.w3.org/2000/svg" onClick={()=>setShowPassword(!showPassword)}
                            viewBox="0 0 640 512">
                                <path fill="currentColor"
