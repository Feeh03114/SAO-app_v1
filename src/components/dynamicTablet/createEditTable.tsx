...
export function CreateEditTable({
    ...
    return(
        <div className="w-full bg-white dark:bg-gray-600 dark:text-white">
            ...
            <div className="grid grid-cols-12 w-full overflow-y-hidden">
                {metadata.fields.map((column) => (
                    <div
                        className={`m-3 ${gridSpan(column)}`}>
                            ...
                            <div className="grid grid-cols-3 gap-x-4 ">
                                {column?.options?.map((option) => (
                                    <div className={`col-span-1 inline-flex items-center`}>
                                        <input 
                                            onChange={(e)=>setData({...data, [column.property]: e.target.value})}
                                            checked={data[column.property] === option.value}
                                            value={option.value}
                                            type="radio"
                                            disabled={column?.disabled}
                                            name={option.value}
                                ...
                                <div className="flex items-center">
                                    ...
                                    >
                                        <span
                                            ...
                                            style={{transform: `${data[column.property] ? 'translateX(100%)' : 'translateX(-100%)'}`}}
                                        />
                                    </button>
                            </div>
                            )    
