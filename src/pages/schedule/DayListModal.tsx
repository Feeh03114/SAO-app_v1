                    ...
                    <div className="flex min-h-full items-center justify-center p-4 text-center md:items-center md:p-0">
                        ...
                        /> 
                            ...
                            <div className="flex flex-row justify-between items-center w-full py-3 md:py-0 px-4 ">
                                    ...
                                    <div className="md:hidden text-base text-slate-900 dark:text-white w-auto">{eventsForDay?.length > 0 && dayjs(eventsForDay[0].dateScheduled, "YYYY-MM-DDTHH:mm:ss.SSSZ").format('DD/MM/YYYY')}</div>
                                    <div className="hidden md:flex text-base text-slate-900 dark:text-white w-auto">{eventsForDay?.length > 0 && dayjs(eventsForDay[0].dateScheduled, "YYYY-MM-DDTHH:mm:ss.SSSZ").format('DD/MM/YYYY')}</div>
                                </div>

                                <div className="grid grid-cols-5 md:grid-cols-6 mt-4 md:ml-4 p-2 rounded-t-lg shadow bg-slate-50 py-2 dark:bg-slate-700">
                                    <p className="col-span-3 md:col-span-1 mt-0 pl-3 md:pl-0 flex justify-start items-center dark:text-white text-slate-500 text-xs leading-5 font-medium">
                                        NOME
                                    </p>
                                    <p className="hidden md:col-span-1 mt-0 md:flex justify-start items-center dark:text-white text-slate-500 text-xs leading-5 font-medium">
                                        SERVIÇO
                                    </p>
                                    <p className="col-span-2 md:col-span-1 mt-0 flex justify-start items-center dark:text-white text-slate-500 text-xs leading-5 font-medium">
                                        HORÁRIO
                                    </p>
                                    <p className="hidden md:col-span-2 mt-0 md:flex justify-start items-center dark:text-white text-slate-500 text-xs leading-5 font-medium">
                                        STATUS
                                    </p>
                                </div>
                                ...
                                <div className="max-h-96 isolate overflow-hidden overflow-y-auto">
                                    ...
                                    {isLoading === false && eventsForDay?.map((value: any, index) => {
                                        return (
                                            <>
                                                <div key={index} className="grid grid-cols-5 md:grid-cols-6 md:ml-4 py-1 px-3 border-b-2 dark:border-slate-600">
                                                    <div className="col-span-3 md:col-span-1 flex justify-start items-center dark:text-white text-slate-900 text-sm leading-5 h-16 font-medium">
                                                        {value.patient.name} {value.patient.lastName}
                                                    </div>
                                                    <div className="hidden md:col-span-1 md:flex justify-start items-center dark:text-white text-slate-500 text-sm leading-5 h-16 font-normal">
                                                        {value.service.name}
                                                    </div>
                                                    <div className="col-span-2 md:col-span-1 flex justify-around md:justify-start items-center dark:text-white text-slate-500 text-sm leading-5 h-16 font-normal">
                                                    ...
                                                    <div className="hidden md:col-span-2 md:flex justify-between my-3">
                                                        <select 
                                                            ...
                                                            className="w-full cursor-text rounded-lg px-4 py-2 dark:bg-slate-700 dark:text-white shadow border border-slate-300 text-slate-900 placeholder-slate-500 focus:border-teal-400 focus:outline-none focus:ring-teal-400 text-sm"
    ...
    <>
        <div className="col-span-6 grid grid-cols-2">
            <div className="col-span-2">
                <label className="pl-4 dark:text-white text-slate-500 text-xs leading-5 font-medium">STATUS</label>
                <select
                    value={value.status}
                    onChange={(e) => update(index, { ...value, status: e.target.value })}
                    className="w-full col-span-2 cursor-text rounded-lg px-4 py-2 dark:bg-slate-700 dark:text-white shadow border border-slate-300 text-slate-900 placeholder-slate-500 focus:border-teal-400 focus:outline-none focus:ring-teal-400 text-sm"
                    placeholder="Não definido"
                    disabled={isLoading}
                >
                    <option value="" selected disabled>Não definido</option>
            ...
            >
                <div className="dark:text-white text-slate-500 text-sm leading-5 font-medium">Procedimento: {value.service.name}</div>
